...
class HttpLink extends Link {
  ...
  Stream<Response> request(
    ...
    final alice_graphqlInstance = GetIt.I<Alice>();

    alice_graphqlInstance.onGqlClientRequest(
      request,
    );
    ...
    alice_graphqlInstance.onGqlClientResponse(
      request,
      httpResponse,
    );
    if (httpResponse.statusCode >= 300 || (response.data == null && response.errors == null)) {
      alice_graphqlInstance.addLog(
        AliceLog(
          message: 'HttpLinkServerException',
          error: httpResponse,
  ...
  http.BaseRequest _prepareRequest(Request request) {
    ...
    final httpBody = _encodeAttempter(
      ...
      (Map body) => json.encode(
        ...
        toEncodable: (dynamic object) => (object is http.MultipartFile) ? null : object.toJson(),
      ),
    )(body);
